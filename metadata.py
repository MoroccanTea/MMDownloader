...
def add_metadata(source_folder="downloaded_music", output_folder="finalized_music"):

    GENIUS_API_CLIENT = os.getenv("GENIUS_API_CLIENT")
    ...
    for filename in os.listdir(source_folder):
        if filename.endswith(".m4a"):
            ...
            if data["resultCount"] > 0:
                ...
                # length = track_info.get("trackTimeMillis", "") # Unused for now
                cover_url = track_info.get("artworkUrl100", "").replace("100x100", "600x600")

                # Fetch lyrics using the Genius API
                genius = lyricsgenius.Genius(GENIUS_API_SECRET)  # Replace with your Genius API key
